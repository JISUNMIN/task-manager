...
- 검색/필터 화면은 "일부 작업만 보여주는 탐색 뷰"로 보고, 원본 순서가 흔들리지 않도록 **필터 적용 중에는 이동/추가를 제한하는 안전한 UX**를 선택했습니다.
...
- **필터 상태에서 드래그 비활성화**
  검색/담당자/우선순위/마감 조건이 적용된 화면은 전체 작업 목록이 아닌 일부 결과만 보여주는 뷰입니다.  
  이 상태에서 드래그를 허용하면 화면에 보이지 않는 작업들 사이의 실제 삽입 위치가 모호해질 수 있어,  
  필터가 적용된 동안에는 작업 이동과 추가를 비활성화하도록 설계했습니다.
- **데모 계정 및 시드 데이터 제공**
